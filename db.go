...
func (db *DB) Get(key []byte) ([]byte, error) {
	...
	//从内存数据结构中取出key对应的索引结构信息
	logRecordPos := db.index.Get(key)

	//如果key不存在内存索引，说明key不存在
	if logRecordPos == nil {
...
func (db *DB) appendLogRecord(logRecord *data.LogRecord) (*data.LogRecordPos, error) {
	...
	//判断当前活跃文件是否存在，因为数据库在没有写入的时候没有文件生成的
	//如果为空则初始化数据文件
	...
	if db.activeFile.WriteOff+size > db.options.DataFileSize {
		...
		//当前活跃文件转换为旧的数据文集，将其放入旧的数据文件map中
...
func (db *DB) setActiveDataFile() error {
	var initialFielId uint32 = 0
	if db.activeFile != nil {
		initialFielId = db.activeFile.FileId + 1
	}
	//打开新的数据文件
	dataFile, err := data.OpenDataFile(db.options.DirPath, initialFielId)
	if err != nil {
		return err
	}
